...
if __name__ == '__main__':
    ...
    topoinf_calculator = None
    if args.model not in ['BERNNET', 'GPRGNN']:
        if args.coefficients == 'last':
            coefficients = [0.0] * (args.k_order + 1)
            coefficients[-1] = 1.0
        else:
            coefficients = [1.0/(args.k_order + 1)] * (args.k_order + 1)
    ...
    for run_index in range(args.n_runs):
        ...
        best_eval_result = RunExp(data, model, args, criterion, 
                                ...
                                dropedge=True, topoinf_all_e=topoinf_all_e)
