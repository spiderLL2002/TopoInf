...
from base_utils.base_training_utils import eval, print_eval_result,calculate_entropy
...
if __name__ == '__main__':
    ...
    for run_index in range(args.n_runs):
        ...
        seed = 2023
        fix_seed(seed)
        entropy_dict = calculate_entropy(best_model, data)
        #print(entropy_dict)
        degree_delete_edge_dict = defaultdict(int)
        """
        entropy_values = list(entropy_dict.values())
        bins = np.arange(0, 2.2, 0.2)  
        hist, _ = np.histogram(entropy_values, bins)
        plt.figure(figsize=(12, 6))
        plt.bar(bins[:-1], hist, width=0.2, align='edge', edgecolor='black')
        plt.xlabel('Entropy Range (bins of 0.2)')
        plt.ylabel('Count')
