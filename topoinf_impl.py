...
def _topoinf_single_edge(edge ):
    ...
    if has_edge: 
        edge_weight = norm_adj_original_sub_g[0, 1].coo()[2] if norm_adj_original_sub_g[0, 1].numel() > 0 else torch.tensor(0.0)
    else: 
        degree_i = _G_g.degree(v_i) + _with_self_loops_g
        degree_j = _G_g.degree(v_j) + _with_self_loops_g
        edge_weight = degree_i * degree_j 
        edge_weight = edge_weight + bool(edge_weight)  

    edge_value = -edge_weight if has_edge else torch.tensor([edge_weight]).pow(-0.5)
    edge_value = edge_value.expand(2)  
